...
class MlKitMedicineImageTextRecognizer implements MedicineImageTextRecognizer {
  ...
  Future<List<String>> extractCandidates({required XFile image}) async {
    ...
    try {
      ...
      final candidates = _buildCandidates(recognizedText);
  ...
  List<String> _buildCandidates(RecognizedText recognizedText) {
    ...
    for (final segment in segments) {
      for (final candidate in _candidatesFromSegment(segment)) {
  ...
  List<String> _candidatesFromSegment(String segment) {
    ...
    final filteredTokens = tokens.where(_isUsefulToken).toList(growable: false);
    final candidates = <String>[];
    final seen = <String>{};
